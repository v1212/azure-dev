...
func TestCreateVoiceAgentAPIRequestFlat_AdvancedSettingsWireShape(t *testing.T) {
	t.Parallel()
	inRate := 16000
	outRate := 24000
	threshold := 0.6
...
func TestCreateHostedVoiceAgentAPIRequestFlat(t *testing.T) {
	t.Parallel()
	store := false
	agent := VoiceAgent{
	...
	req, err := CreateHostedVoiceAgentAPIRequestFlat(agent, agent_api.VoiceTargetAgentReference{
...
func TestCreateHostedVoiceAgentAPIRequestFlatRequiresResolvedTarget(t *testing.T) {
	t.Parallel()
	agent := VoiceAgent{
		AgentDefinition: AgentDefinition{Kind: AgentKindPromptVoice, Name: "voice-wrapper"},
		ModelType:       VoiceModelTypeHostedAgent,
	}
	if _, err := CreateHostedVoiceAgentAPIRequestFlat(agent, agent_api.VoiceTargetAgentReference{}); err == nil {
