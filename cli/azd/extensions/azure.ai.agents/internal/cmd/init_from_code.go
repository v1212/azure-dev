...
func (a *InitFromCodeAction) Run(ctx context.Context) error {
	...
	if existing, statErr := findExistingAgentYaml(srcDir); statErr == nil && existing != "" &&
		!isHostedVoiceSourceDescriptor(existing) && !strings.EqualFold(a.flags.kind, kindFlagHostedVoice) {
...
func (a *InitFromCodeAction) addHostedVoiceWrapper(ctx context.Context, targetServiceName string) error {
	...
	projectServiceName := existingProjectServiceKey(ctx, a.azdClient)
	...
	if _, err := a.azdClient.Project().AddService(ctx, &azdext.AddServiceRequest{Service: &azdext.ServiceConfig{
		Name:                 wrapperName,
		Host:                 AiAgentHost,
	...
	if err := setServiceUses(ctx, a.azdClient, wrapperName, []string{projectServiceName, targetServiceName}); err != nil {
		return err
	}
